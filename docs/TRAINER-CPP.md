...
### 6.18 P5 世代整条跑在 C++ 生产者上（实测：采集 40.3 s vs Java 730–833 s）

**为什么这一步才是终点**：P5 的采集席 = 2 席自己（`net:<上一代>#<T>`）+ 2 席联赛对手（**老师常驻一席**），
所以它同时需要 **`net:` 的 `#T` 采样**（§6.16）与 **`teacher`**（§6.17）—— 两者齐了，"世代上 C++"才谈得上。

```powershell
$env:MAHJONG_PRODUCER = 'cpp'   # 其余参数与 Java 生产者一字不差
cd python; .\.venv\Scripts\python.exe -m mahjong_ml.online run `
  --init S:\mahjong-training\ckpt\ppo2-g04\model.pt --label ppo2 --start-gen 5 --generations 5 `
  --gen-games 2000 --workers 16 --seed 701001
```

**第 5 代实测（2000 场完整半庄，对手 `teacher,random`）**：

| 步骤 | 生产者 | 用时 | 备注 |
| --- | --- | --- | --- |
| ① 采集 2000 场 | **C++** | **40.3 s** | 23,683 小局 / **1,518,765 决策** / **49.6 场/秒**（37,707 决策/秒，16 workers） |
| （对照）同一批采集 | Java | 730–833 s | g01–g04 台账口径（同 2000 场）⇒ **≈18–20×** |
| ② 派生特征 2000 个 | C++ | 9.9 s | 300.2 MB sidecar |
| ③ 紧凑集 | Python | 452.6 s | 1,364,209 训练条 / 154,556 验证条（磁盘侧，与生产者无关） |
| ④ PPO 4 epochs | Python/GPU | 125.7 s | val MAE **5.32**（常数基线 6.32）、逐点 ρ +0.718 / episode ρ +0.761 |
| ⑤ 导出 `net.bin` | Python | 3.1 s | 1,067,040 B / 266,753 参数（与 Java 侧同格式） |

**数据质量（独立校验器）**：`node tools/selfplay-check.mjs S:\mahjong-training\raw\ppo2-g05` → **DATASET PASS**
（2000 场 / 23,683 小局 / 1,518,765 决策；动作分布 `discard=1167429 pass=259325 riichi=25312 pon=23328
chi=20305 ron=14893 tsumo=5159 kan=3014` —— teacher 席把**开杠/自摸**这些分支也带进了整场数据，
这是 `first` 时代的采集里没有的）。

**判据（强度）不在训练日志里** —— `online ladder --label ppo2 --generations 5`（每代 1200 场 Elo 跑
+ 800 场 2+2 同牌山配对，全部走 C++ 生产者；这 10,000 场采集只用了 **≈2.5 分钟**，Java 同量级约 1 小时）：

| 代 | Elo θ（锚 teacher） | θ−teacher | 配对 Δ 顺位点 | 95% CI | p | 胜/负/平 |
| --- | --- | --- | --- | --- | --- | --- |
...
| first / random | −2.714 / −2.735 | −3.4 / −3.4 | — | — | — | — |

**怎么读（照 `league.py` 的口径）**：绝对 θ 只在"同一批对手"下可比，判据是 **θ−teacher** 那一列；
**g05 是第一代配对 CI 不含 0 的**（+4.90 [+1.38, +8.46]），且 5 代的 θ 与 Δ **单调抬升**。
...
**2026-09-26 的算力口径更新（紧凑集并行化之后）**：一代 2000 场的实测从 **≈11 min** 降到 **≈3.6 min**——
采集 51.0 s（16w，C++）+ 派生特征 9.2 s + **紧凑集 45.0 s**（原 459–517 s，`dataset build` 已按文件分组并行、
