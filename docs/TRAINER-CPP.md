...
的第 **119** 场就会踩到一个约 **1/100 场**才出现的退化局面（食替把暗手每种牌都禁打 → `legal` 为空，
...
**现象**（完整半庄约 **1/100 场**出现一次）：`selfplay` 跑到某一场突然报
