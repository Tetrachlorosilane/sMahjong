...
| 文件 | 来源 | 许可 |
| --- | --- | --- |
| `Qt6Core.dll` `Qt6Gui.dll` `Qt6Widgets.dll` `Qt6Network.dll` `Qt6Svg.dll` | Qt 6.11.2 (mingw_64) | **LGPLv3**（或 GPLv3 / 商业，三选一） |
| `platforms\qwindows.dll` `styles\qmodernwindowsstyle.dll` `tls\*.dll` | Qt 插件（属 Qt Base） | **LGPLv3** |
...
（只取 qtbase + qtsvg，详见 `client/README.md`）。两者是同一份官方产物，许可结论完全一致。
...
| **② 随附许可文本与版权声明** | `client/licenses/`：`Qt-LICENSE.txt`（Qt 官方 LICENSE，含 **LGPLv3 全文 + GPLv3 全文**；LGPLv3 是 GPLv3 的补充条款，必须一并提供）、`Qt-Copyright.txt`（第三方版权声明）、`NOTICE.txt`（本项目的说明）。`build.ps1 -Deploy` 会自动把它们拷进 `client/dist/licenses/` |
...
- **新增 Qt 模块**：先确认它不在 §2 的"仅 GPLv3"名单里；若是，要么换成 LGPLv3 的替代方案，
  要么整体改用 GPLv3/商业授权。同时更新三处：`client/build.ps1` 的必需 DLL 清单、
  `tools/qt-provision.ps1` 的 `$script:QtDefaultModules`（决定自动下载取哪些模块归档）、
  以及 `client/licenses/NOTICE.txt` 里的模块列表。
