...
| **S-53** | low | **多家荣和的 `agari` 报文里 `scores_after` 是"中途快照"**：`agariRon` 在循环里**边结算边发**（每个赢家发完才轮到下一个），所以二家以上时先发的那条 `scores_after` 里还没有后一家的收支 | `Round.agariRon` 的 `applyDelta` / `sendAgari` 顺序 | 客户端按顺序处理、结算界面取**最后**一条 `agari`，所以现在看不出问题；但回放/重连若取中间那条会看到不完整的分数。修法：先全部 `applyDelta` 再统一发（会改变报文顺序，要连着回放与 L3 一起验） |
