...
| **配额副作用（照实记）** | `compact` 配额 10 GB 的滚动淘汰 | 四代紧凑集（~1.5 GB/代）+ 旧数据集把配额顶爆 ⇒ **`compact/rl-001`（P3 的数据集）被淘汰**；`raw` 里三个来源（`bc-001`/`dagger-01`/`control-r1`）仍在（raw 9.67/30 GB），重建命令：`dataset build raw/bc-001 compact/rl-001 --src raw/dagger-01 --src raw/control-r1 --val-frac 0.1 --split-seed 20260401`（≈13 分钟） |
<!-- /P4-ROUND -->
