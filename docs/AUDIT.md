...
| **S-58** | `fourKanAbortNow()` 移到**鸣牌落地之前**（荣和先 `return`，三种豁免不受影响）；删掉 `isRinshan` 分支里那句死代码 + 错理由码 | ⚠ **无自动化断言**：要定向构造"4 杠局面 + 一次可鸣的岭上舍张"才能测；目前靠 `fourKanAbortTests` 的真值表 + 调用顺序。**已知缺口** |
...
**本轮仍未闭合并已登记为已知限制**：
- **S-58 的时序**（abort 早于鸣牌落地）：代码已按文档改对，但**缺自动化断言**（见上）。
