...
| **P4 第二轮（`ppo2`：`lr` 1e-4→3e-4、`epoch` 4→8，其余全同）** | `online run --label ppo2 --generations 4` + `online displace` | **步长确实提上去了**：每代位移 **4.83% / 5.59% / 5.00% / 5.19%**（第一轮 2.68/2.56/2.66/2.42% ⇒ 约 2 倍），首→末累计 **9.49%**；**KL 早停每代都触发**（5/3/4/3 epoch，阈值 0.03）⇒ 步长上去后信任域成了限制；价值头继续变好（MAE 5.1715~5.4378 全 < 常数基线，小局级 ρ 0.711→**0.779**） |
| **P4 判据（两轮联合 PL 拟合，9200 场 / 11 策略，配对自助法 200 次）** | 8 次阶梯 run 合并 + `league.boot_thetas`/`ci_from_thetas` | ✅ **Elo 单调上升在第二轮内部成立**：四代都显著高于 `teacher`（+0.150/+0.334/+0.333/+0.371，CI 全排除 0），**`ppo2-g04 − ppo2-g01 = +0.221`（CI [+0.075,+0.358]，p=0.010）**，而第一轮同口径只有 +0.058（p=0.51）⇒ 步长是限制因素，已修。⚠ 但**第二轮整体 ≈ 第一轮**（`ppo2-g04 − ppo-g04 = +0.139`，CI [−0.014,+0.305]，p=0.110）⇒ **位移翻倍 ≠ 强度提升**。✅ 对脚本基线不掉：`first` −3.416 / `random` −3.470（全排除 0） |
| **P4 判据（2+2 头对头，模型无关）** | `online pair`：两批独立 run（1500 + 3600 场，seed 1102001/1202001）+ `eval --pool` | 🏁 **`ppo2-g04 − teacher` 合并 n=5100 得 Δ=+2.290 顺位点，95%CI [+0.85,+3.71]，符号检验 p=0.006**（两批 +1.93 / +2.44，**批间 τ=0.00**；run 级随机效应 CI [+0.85,+3.73] 仍排除 0）⇒ **本项目第一次有策略在头对头上显著超过 `teacher`**（此前最好 `hybrid@1 − teacher = +1.20`、p=0.977 打平）。⚠ 样本量是**按 §7 功效口径预先算的**（`required_n(sd=52.5, Δ=2)=5408`，实跑 5100） |
| **P4 判据（相对自己的初始化）** | `online pair --a ppo2-g04 --b awr-002 --games 3600` | 🏁 **Δ=+1.87 顺位点，95%CI [+0.13,+3.58]，p=0.039**（1861/1736/3，sd=53.05）⇒ **"比老师强"是 P4 挣来的，不是起点就有的**（起点 `awr-002` 就是 P3 的终点）。⚠ 链路：第二轮从 `ppo-g04` 初始化，所以这一格是**两轮累计**增益，而上一行说明它**在第一轮就已全部拿到** |
| **P4 判据（第二轮 vs 第一轮）** | 同上，`ppo2-g04` vs `ppo-g04`（1500 + 3600 场）+ `eval --pool` | ⚠ **打平**：合并 n=5100 得 **Δ=−0.193，95%CI [−1.62,+1.30]，p=0.183**（两批 −0.14 / −0.21，τ=0.00）⇒ **不仅没变强，CI 还把"强过 +1.6"排除了**；θ 口径也只差 +0.139（p=0.110）⇒ **位移翻倍（4.9%→9.5%）一点强度都没换来** —— 「位移只能证明动了，不能证明变强了」的对照实验 |
| ⛔ **存储事故（2026-09-25 05:05）训练盘掉总线** | 第二轮判据评测（`online pair`）写 `raw/ppo2-g04-vs-ppo-g04` 时 | 先 `java.nio.file.AccessDeniedException`（写）→ 再 `ERROR_IO_DEVICE`（读）→ 随后 **`S:`（Silicon_files）与 `E:`（Silicon_SSD）两个卷一起从系统消失**（`[System.IO.DriveInfo]::GetDrives()` 只剩 C:/D:）。**诊断教训**：症状像权限问题（`EPERM`/`AccessDenied`），我一度按"沙箱收紧"处理 —— 正确顺序是**先查盘符**。**影响面**：`C:` 工作区的代码/文档不受影响；第二轮四代 checkpoint 与轨迹已写完（可恢复性取决于重新插拔）。**待办**：盘回来后 ① `paths.ensure_root()` 探针 + `selfplay-check` 抽查；② 补跑 `pair ppo2-g04 vs ppo-g04` / `vs teacher` + `ladder --label ppo2`；③ 若反复掉线则换卷（`MAHJONG_DATA_ROOT` 一行）。**✅ 已恢复（同日）**：重新插拔后四卷齐备，**全树 55,894 个文件逐个读头 8 KB、读失败 0**；掉线时唯一报 `ERROR_IO_DEVICE` 的 `ckpt/ppo2-g01/{metrics.json,net.bin}` 现在都读得出、`metrics.json` 解析结果与控制台日志逐项一致（5 epoch / 934,761 决策 / valMAE 5.3834）⇒ **数据没坏**。恢复次序（照上面写的做了一遍）：写探针 → **删掉中断的半截 run**（`raw/ppo2-g04-vs-ppo-g04` 只剩 174 文件；不删会被同名重跑当成本次 run 的一部分）→ 60 场烟雾 + `selfplay-check` **DATASET PASS** → 才开评测 |
 | **判据① Elo 阶梯** | `online ladder --generations 4 --games 1200`（四代结构相同，共 4800 场）+ 分层自助法 200 次配对 | **四代彼此不可区分、无单调趋势**：`g04−g01 = +0.058`（CI [−0.099,+0.205]，p=0.51）、`g02−g01 = +0.065`（p=0.44）、`g03−g01 = +0.016`（p=0.85）、`g04−g02 = −0.007`（p=0.96）⇒ **当时判据「Elo 单调上升」不成立**。⚠ **后续修正（第二轮跑完后的两轮联合拟合）**：第一轮**末代的水平**其实已经显著高于 teacher（`ppo-g04 − teacher = +0.232`，CI [+0.096,+0.368]）—— 所以准确说法是"第一轮**看不出进步**"（1100 场/代 ≈ ±0.15 θ + 每代只走 2.5% 决策），不是"第一轮没效果" |
