...
- ⚠ **`score_delta` 与 `scores_after` 分工**（两个都在，别混用）：
  `score_delta` 只是**本局结算的收支** —— 和了点数的授受 + 赢家收走的供託，
  **不含各家自己立直宣言时扣掉的那 1000 点**（宣言那一刻就已经扣了，见 `/draw` 与立直流程）；
  它的和可以是 +1000×收棒数（示例里的 `[8000,-2000,-2000,-2000]` 和 = +2000 就是这种情形）。
  **要「结算后的权威四家点数」一律读 `scores_after`**（流局事件同样带），
  它已经把立直投入、燕返退回、供託全部算进去，且与本局之后 `round_start.scores` 逐位相等。
  需要十鳳牌譜那种「本局四家净增减」就自己算 `scores_after − 本局起始点数`
  （牌谱导出就是这么写的，抄 `score_delta` 会让点数板与差分对不上 —— 见 NOTES §9.6）。
...
> 算力预算、每阶段的验收判据 —— 见 **`docs/TRAINING.md`**（P0–P4 与 P5b 已落地并附实测）。
