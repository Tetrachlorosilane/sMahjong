...
- **配对口径**：每代另跑一批 **2+2**（`net×2 vs teacher×2`）给顺位点的逐场配对 CI ——
...
- **算力（2026-09 实测，C++ 生产者）**：**一代 = 2000 场完整半庄**（`--gen-games` 缺省值；2 席自己 +
  ...
  **判据另跑**（`online ladder`）：每代 1200 场（Elo）+ 800 场（2+2 配对），实测每批 13.9–15.4 s
  ⇒ 5 代共 10,000 场只用 **145 s** 采集，加 `eval.py` 与 Plackett-Luce 统计约 **3–5 分钟**
  （Java 侧同量级约 1 小时）。⇒ **"一天能跑几代"现在由紧凑集 + PPO 决定**，采集已不是瓶颈。
