...
2. ✅ **判据「Elo 相对 teacher 单调上升」在第二轮内部成立**：四代**都**显著高于 `teacher`
...
3. ⚠ 但**第二轮整体 ≈ 第一轮**：`ppo2-g04 − ppo-g04` 合并 n=5100 得 **Δ = −0.193 顺位点，
...
4. ✅ **判据「对脚本基线不掉」成立**：`first` −3.416 / `random` −3.470（相对 `teacher`），
   与全部网络（≥ +0.150）差着一整个量级 ⇒ 没有"过拟合到自己的策略"的迹象。
5. **还差最后一格（下一轮第一件事）**：**`ppo2-g04 − awr-002`（相对自己的初始化）** ——
   现在只证明了"比老师强"，还没证明"**是 P4 让它比老师强的**"（起始点 `awr-002` 本身可能就已经
   接近老师）。这一格跑 3600 场即可分辨 Δ≥2（`sd≈52.5`）。
