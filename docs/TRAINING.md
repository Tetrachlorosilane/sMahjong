...
- **配额闸门（写之前先查剩余，超了滚动淘汰最旧的，raw 先淘汰）**：
  `raw ≤ 30 GB`、**`compact ≤ 50 GB`**、`ckpt + league ≤ 3 GB`、`logs ≤ 1 GB`，
  并且**任何时刻保留 ≥ 10 GB 余量**（jsonl 写满盘会让采集静默失败）。
  ⚠ **`compact` 从 10 GB 提到 50 GB**（2026-09，用户指定）：P4 的世代循环每代一份紧凑集（~1.5 GB），
  四代就把 10 GB 顶爆，滚动淘汰**按 mtime 从最旧的开始删**、不区分"废弃数据集"与"还要复算的数据集"，
  于是把 `compact/rl-001`（P3 的数据集）删了。提到 50 GB 后 4 代的余量从 ~6 代涨到 ~30 代。
...
**判据（Elo / 2+2 配对）—— ⛔ 本轮未取到，原因是存储掉线（不是训练失败）**：
第二轮四代的 checkpoint 与轨迹都写完了，但**判据那批评测（`pair` / `ladder`）刚开跑就撞上数据盘掉线**
（见 `NOTES.md` §6.5「存储事故」）：`S:` 先写不进（`AccessDeniedException`）、再读不出
（`ERROR_IO_DEVICE`），随后 **`S:` 与 `E:` 两个卷一起从系统里消失**。所以第二轮**只有**
"位移 / KL / 价值"三组证据，**"Elo 是否上升、对 `teacher` 与对第一轮是否更强"仍待补**：
盘回来后先跑 `online pair --a ppo2-g04 --b ppo-g04 --games 1500` 与 `--b teacher`，再跑
`online ladder --label ppo2`。

⚠ 因此**不要**把"第一轮的 Elo 不上升"当成 P4 的最终结论 —— 第一轮已查明是**步长太小**
（每代 2.5%），第二轮的步长修好了（每代 5%），只是判据还没读出来。
