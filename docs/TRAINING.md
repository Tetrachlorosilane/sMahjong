...
- **配额闸门（写之前先查剩余，超了滚动淘汰最旧的，raw 先淘汰）**：
  `raw ≤ 30 GB`、**`compact ≤ 100 GB`**、`ckpt + league ≤ 3 GB`、`logs ≤ 1 GB`，
  并且**任何时刻保留 ≥ 10 GB 余量**（jsonl 写满盘会让采集静默失败）。
  ⚠ **`compact` 10 → 50 → 100 GB**（2026-09，用户两次上调）：P4 的世代循环每代一份紧凑集（~1.5 GB），
  四代就把 10 GB 顶爆，滚动淘汰**按 mtime 从最旧的开始删**、不区分"废弃数据集"与"还要复算的数据集"，
  于是把 `compact/rl-001`（P3 的数据集）删了。100 GB ≈ 60 代的余量。
  ⚠ **`enforce_quota` 是在 `allocate()` 时检查的**，而目录是之后才长大的（`dataset build` 要几分钟），
  所以**实际占用会超出配额一代的量**——看 `paths.report()` 时别把那当成 bug。
  ⚠ 盘的**总容量**仍是最终闸门（`MIN_FREE_GB`）：配额放大不等于可以无限堆，S 盘约 248 GB。
...
1. ✅ **迁到 S 盘后，"写盘"这件事基本不花钱了**（迁盘后实测，同一版代码/同样 24 workers/全量轨迹）：
   ...
   **2.6 GB / 40 s**，S 盘 `raw` 配额 30 GB（§0.1.1）变成唯一的约束 ⇒ 采完**立刻**转紧凑格式再删 `raw`
