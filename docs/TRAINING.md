...
| 对局吞吐（**迁盘后实测，2026-09**） | **1.44 场/秒**＝不落盘的纯算力上限；**写数据盘 S: 1.42 场/秒**（全量轨迹 429.6 MB/400 场）；**写工作区 C: 1.23 场/秒** —— 见下面第 1 条 |
...
1. ✅ **迁到 S 盘后，"写盘"这件事基本不花钱了**（迁盘后实测，同一版代码/同样 24 workers/全量轨迹）：
   **不落盘 1.44 场/秒 → 写 S 盘 1.42 场/秒**（≈1%，噪声内）；反倒**写工作区盘 C: 只有 1.23 场/秒**（−15%）。
   ...
   按 1.4 场/秒 ≈ **5.1k 场/小时** 规划；`--sample 1` 约 **7.8 GB/小时**，S 盘 `raw` 配额 30 GB（§0.1.1）
   ≈ 3.8 小时连采 —— 采完**立刻**转紧凑格式再删 `raw`（`compact` 配额 10 GB 也要盯）。
...
- **顺位点（rank_points）的尺子**（实测 `sd(Δ) ≈ 53`，来自 `net vs teacher` / `AWR vs BC` 的配对差）：
  ...
  **50% 功效**（效应恰好压在临界值上、一半概率检不出），把所需场次**低估了 2.04 倍**
  ...
  2.6k 场 ≈ **半小时**（迁盘前写 `T:` 时要一小时，见 §0.1.1）。**做评测跑不需要全量轨迹** ——
