...
- **结果（两轮，同数据同评测协议）**：
  ...
  | --- | --- | --- | --- | --- | --- | --- |
  | `awr-001`（λ=0 critic）− BC，2400 场 | +0.92 | [−1.21, +3.03] | 0.391 | 1221 / 1178 / 1 | 52.88 | ~2686 场 |
  | `awr-001`（λ=0 critic）− `teacher`，1200 场 | −1.16 | [−4.11, +1.86] | 0.977 | 601 / 599 / 0 | 52.47 | ~2644 场 |
  | **`awr-002`（λ=1 critic）− BC**，2400 场 | **+1.27** | **[−0.79, +3.39]** | **0.094** | 1241 / 1158 / 1 | 52.96 | ~2695 场 |
  | **`awr-002`（λ=1 critic）− `teacher`**，1200 场 | −1.66 | [−4.59, +1.31] | 0.885 | 597 / 603 / 0 | 51.93 | ~2590 场 |
...
- **裁定：判据② 仍未达到显著，但方向一致、且换用"排序更好"的 critic 后效应更大。**
  `AWR − BC` 从 λ=0 的 **+0.92（p=0.39）** 提到 λ=1 的 **+1.27（p=0.094）**；
  对 `teacher` 略负（−1.66，p=0.89，胜 597/负 603 = 完全打平）。要把它钉到 95% 需要 ~2700 场/臂。
  ⚠ 把已知的尺子摆在一起看：`BC − teacher ≈ −1.6`、`hybrid@1 − BC ≈ +1.95`、`AWR − BC ≈ +1.3`
  —— **全部落在 ±2 顺位点内**。结论：到 P3 为止，**没有任何改动被证明在实战里变强**；
  差别都在噪声里，而"离线更像老师 / 排序更准"与"实战更强"是三件不同的事。
- **下一步（按性价比）**：① 把 AWR 的**优势权重**换成按小局归一（现在按批内 σ 归一，但一条 episode 内
  ...
  （策略梯度直接优化我们真正测量的东西）；④ 若要继续这条线，先把 `AWR − BC` 跑到 ~2700 场/臂
  把 +1.27 钉到显著（约 1 小时）。
