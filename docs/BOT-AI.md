...
| 包 | kind | 说明 |
| --- | --- | --- |
| `teacher` / `first` / `pass` / `random` | builtin | 内置锚点（启发搜索与三个对照臂）—— **与特征版本无关**，任何一版都能用 |
| `bc-003` | net | BC 基线（行为克隆）· ⚠ **v2 权重（607 维），v1.10.6 起不再随发布包附** |
| `awr-002` | net | 离线 RL（AWR）：**比 BC 显著强**（+1.89 顺位点，p<0.001，n=12000）· ⚠ 同上，v2 权重 |
| `ppo-g04` | net | 在线 PPO 第 1 轮第 4 代 · ⚠ v2 权重 |
| `ppo2-g04` | net | 在线 PPO 第 2 轮第 4 代（**纯网络**；资产里那份历史上曾带 `alpha: 2` = 让位 95.1%）· ⚠ v2 权重 |
| `ppo2-g05` | net | 在线 PPO 第 2 轮第 5 代（**纯网络，无先验**）：2+2 同牌山配对 **+4.90 顺位点 vs teacher，CI [+1.38, +8.46]，p=0.010（n=800）** · ⚠ v2 权重 |
| `ppo-v3-g01-g01` | net | **特征 v3 谱系第 1 代**（2026-09-26 起，随 **v1.10.6** 发布）：BC 基模（val top-1 0.876）+ 1 代 PPO；2+2 同牌山配对 vs teacher **Δ=+1.22 顺位点，95%CI [−2.32, +4.69]**（**证不出差别**）、Elo θ−teacher = +0.137（半宽 0.209） |

> ⚠ **特征 v3（v1.10.6 起）与 v2 权重不兼容**：`net.bin` 头部记着 state/cand 维度，
> v2 的 607 维权重会被服务端**构造期拒绝** ——
> `权重维度 (607,96) != 本服务端特征维度 (615,96) —— 重新导出权重（特征版本 3）`。
> 所以 **v1.10.6 的发布资产里只有 `ppo-v3-g01-g01` 这一个深度模型包**（表里带 ⚠ 的那些是 v1.10.5 及更早的资产，
> 只适用于那一版及更早的服务端）。内置的 `teacher / first / pass / random` 不受影响。
