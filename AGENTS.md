...
**teacher 的四层取舍**（牌效/押し引き/打点与役/开杠，见 §6.6），
以及 **3 次「4 机器人整场半庄」**的点数守恒。改了 `rules/` / `game/` 下任何东西都要重跑。

> ⏱ 全量自检约 **100~130 秒**（2026-09 规则收口 + teacher 开杠之后 **1047** 项；审计三轮之后是 944 项 / 115 秒）
...
**实测通过**：服务端自检 **1047** 项（含训练接口不变式 + 振听三条 + teacher 四层取舍 + 批次一的口径修复 + 《雀魂》的国士抢暗杠 / 天和国士 + 包牌多责任者列表）、客户端自检 **678** 项、§4 的全部 L3 工具（含 `replay-test`、
