...
- **UI 扁平化**：纯色 + 1px 描边，**牌片单层外框、不画厚度层**，不用渐变/阴影/发光（`TileRenderer` 的
  程序化绘制只作**回退**保留）。
...
- **牌河一行最多 1 张横置牌**：只有立直宣言牌横置，且每家至多 1 张。所以最坏一行 =
  ...
  布局按**同一函数**的四家最大值预留（`computeLayout` 的 `riverRows`，写进 `m_riverRows`），
  多出来的行才不会压到手牌 —— 两者必须同源，否则要么丢牌要么压牌。
- **风盘区带：四边同构**「立直棒带（贴盘边）+ 点数带（在其内侧）」+ **宝牌行** + 中间（场次·余牌·供託）。
  ...
  - **宝牌指示牌行只占「中间列」**（左右两条竖排点数带之间），**并且不再画「宝牌」字样**：
    那行原来横跨盘内整个宽度、左边还挂着标签，5 张牌会伸出四家得点框围出的那一列。
...
- **河区预留范围 = 风盘向外等距扩一圈**（**只算布局，不绘制** —— 那圈浅白线框已按需求删除）：
  ...
  它在 `computeLayout()` 里算并存成成员（`m_riverArea`），受外圈预算约束：
...
- **报文里不带中文：词汇性文本一律只发 ASCII 码，中文文案只存在于客户端语言文件里**。
  ...
  - **界面固定文案也全在 `ui.*`（173 条）**：按钮/标题/标签/tooltip/结算 HTML 的中文都在语言文件里，
...
**实测通过**：服务端自检 **414** 项、客户端自检 **401** 项、Node 协议 E2E（含**报文 ASCII 审计**与**岭上账**）、
