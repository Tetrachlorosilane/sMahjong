...
- **牌河的横向起点是「固定左缘」，不是「按当前张数居中」**（用户报障过**两次**，别再把这两件事混为一谈）：
  ...
  - 现在的口径：**先假定一行排满**（横置 1 张 + 5 张普通 + 5 个列间距），算出它的最最左沿，
    所有行、所有张数都从那条线**向右**排。实现在 `TableLayout::riverFullRowExtent()` / `riverLeftU()`，
    与已经打了几张**无关**；绘制（`paintRiver`）与飞行动画终点（`riverSlotScreen`）必须读**同一个** `riverLeftU()`。
  - 自检用**局部坐标**断言（`TableView::riverSlotLocalForTest`）：屏幕坐标带着四家的旋转，
    「左沿」在左右两家那里其实是上下方向（`pos=1` 的 `toScreen` 把局部 u 映射到屏幕 y）。
    回归：`client --selftest` 的「牌河固定左缘」组（1/3/6/7/13/18 张左缘不变 + 三行行首同源 + 含旧位置的红证）。
...
- **王牌 14 张 = 4 张岭上 + 5 张表宝牌指示牌 + 5 张里宝指示牌**（`dead[0..3]` / `dead[4..8]` / `dead[9..13]`）。
  ...
  **两条账必须分清**：
  ...
  - ⚠ **机器人在正常对局里从不开杠**（`Bot` 的大明杠是「简化：不开」，出牌段也不看 `kan` 选项）→
    整条岭上路径在整场模拟里**一次都走不到**，上面那个 bug 就是这么漏掉的。
    自检用 `Bot.debugAlwaysKan = true` 临时让机器人开杠，配合 `Table.debugEventTap`
    **抓真实报文**断言（`SelfTest.rinshanTests`：三种杠各出现几次 / 各自之后摸到几次岭上牌）。
...
**实测通过**：服务端自检 **597** 项、客户端自检 **669** 项、§4 的全部 L3 工具（含 `replay-test`、
...
**牌河固定左缘**另有客户端自检的「牌河固定左缘」组（1/3/6/7/13/18 张左缘不变 + 三行行首同源，
并在旧实现上验过会红：第 1 张 −30.39 → 打到 3 张 −79.10，而固定左缘是 −152.16）。
