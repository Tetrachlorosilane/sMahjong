...
- **牌山视图**（「牌山视图」按钮，单独开一个窗口）：136 张按**真实抓牌顺序**铺开 ——
  每行一家、从庄家起，前 12 张是三轮各 4 张（每 4 列一组），再各补 1 张，庄家第 14 张，
  之后是一张一张的摸牌；末尾 14 张是王牌（4 张岭上 + 5 张表宝牌指示牌 + 5 张里宝指示牌）。
  每张牌按**当前进度**着色（牌背＝还没摸到、牌面＝已摸到，灰＝已打出、绿框＝已副露、
  红框＝这一步刚摸到），点某张牌还能跳到"它被摸走"的那一步。
