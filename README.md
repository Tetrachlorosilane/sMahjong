...
下拉框里的**清单由服务端决定**：服务端启动时会**自动挂上 `bot-ai/` 目录**（与 `replays/`、
`players/` 同一层，也就是 `start.sh` 与 `mahjong-server.jar` 所在的那个目录）里的每一个 AI 包 ——
**把发布页给的 zip 解压到服务端目录、重启服务端**，下拉框里就多一代，不用改任何配置
（发布页有「一包全给」的 `sMahjong-bot-ai-*.zip`，也有按代分开的 `ppo2-g04.zip` 这类）。
也可以用 `--bot-ai-dir <目录>`
（把每个含 `net.bin` 的子目录按目录名挂上，比如 `ppo2-g04`）或 `--bot-ai-reg 名字=策略串` 手工挂。
客户端只发**名字**、不发文件路径 —— 所以这个功能不会让房间去读服务器上的任意文件。

各代的来历与强度（哪一代最会用牌、比内置 AI 强多少）见 `docs/TRAINING.md`；
包格式（自己打一个包挂上去）见 `docs/BOT-AI.md`。
...
- **导出天鳳牌谱**：工具栏「导出牌谱」一次写**三份**（同一个名字，只是后缀不同）：
  - `<名字>.json` —— **天鳳 `tenhou.net/6` 原生牌谱 JSON**（配牌、每一次摸打、吃碰杠/暗杠/加杠、
    立直宣言、杠宝牌、里宝、和了/流局**全都在**）。这一份可以直接喂
    [mjai-reviewer](https://github.com/Equim-chan/mjai-reviewer) / Mortal 做 AI 复盘：
    `mjai-reviewer -e mortal -i <名字>.json -a <自己的座位 0-3>`。
    ⚠ Mortal **只支持半庄**：東風戦导出时会把规则名照实写成「東」，让引擎明确拒绝
    （而不是按半庄算出一堆错的期望值）。
    这份 JSON 是**紧凑单行**的 —— 因为网页版 [mjai.ekyu.moe](https://mjai.ekyu.moe/zh-cn.html)
    的「自定义 (tenhou.net/6 JSON)」输入框对请求体有大小门限，缩进版（每个数字一行）会让
    一场半庄涨到 60 KB 以上、被服务端挡成 `400 Bad Request`；紧凑版只有 10 KB 左右。
    想用网页版复盘：把**整个 `.json` 文件的内容**贴进那个框，目标选自己的座位（0-3），提交。
  - `<名字>.txt` —— 同一份牌谱的 `tenhou.net/6` 链接形式：**第一行就是链接**，
    整行粘到浏览器即可查看这一场。
  - `<名字>.xml` —— **完整天鳳牌谱（mjlog XML）**：给天鳳系的分析/AI 工具用。
  ⚠ 和了明细里的「多少符多少飜」只是给人看的文字（解析器不读它，天鳳的役种是数字 id、
