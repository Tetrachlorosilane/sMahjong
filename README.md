...
- **导出天鳳牌谱**：工具栏「导出牌谱」把这一场导出成**天鳳牌譜**（tenhou.net/6 的 `#json=` 链接形式，
  格式参考 [wuye999/tenhou](https://github.com/wuye999/tenhou)）。导出的是一个 `.txt`：
  **第一行就是链接**，整行粘到浏览器地址栏即可在 tenhou.net/6 里查看这一场；下面是同一份 JSON，方便别的工具读。
...
- **个人设置**：大厅右下角「个人设置」里可以改**服务器地址/端口、昵称、材质包**，
  ...
  认不出的项原样保留。
- **材质包**：能把**牌面/牌背、桌布、立直棒、UI 字体**换成自己的素材
  （结算界面那套麻将牌字体不在此列）。
