...
public:
     ...
     * **加杠横置的仍是原碰里被鸣的那一张**（不是新加的第 4 张，也不占新槽位）。
     */
    ...
    /** 自检用：牌河**固定左缘**（局部 u）= 一行排满时最左那张的左沿。 */
    qreal riverLeftUForTest() const { return m_layout.riverLeftU(); }
    /** 自检用：牌河一行排满时的横向宽度（含一张横置牌的额外宽度）。 */
    qreal riverFullRowExtentForTest() const { return m_layout.riverFullRowExtent(); }
