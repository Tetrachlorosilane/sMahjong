...
struct TableLayout {
    ...
    /**
     * 牌河**一行排满**时的横向宽度 —— **按 6 张普通牌 + 5 个列间距算，不含横置牌的额外宽度**。
     *
     ...
     *   ⚠ 口径（用户 2026-09 二次明确）：**横置牌不算进左缘** —— 立直宣言牌占的是**牌高**、
     *   比普通牌宽约 0.36 张牌宽，把它算进去会让**每一行**（哪怕没人立直）右侧都空出那一截、
     *   整条牌河看着偏左。所以现在：普通行正好铺满这条线，**立直那一行允许向右多出
     *   (牌高 − 牌宽)**。一行最多 1 张横置牌，偏移量很小，且只影响立直家自己的河。
     */
     ...
     * （上家=最左、对家=中间、下家=最右）；暗杠不横置；
     * **加杠横置的仍是原碰里被鸣的那一张**（加上的第 4 张另画在它上方）。
