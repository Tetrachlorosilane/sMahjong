...
- **P5b 混合（teacher 先验）**（2026-09）：策略串 `net:<权重文件>@<α>`，语义
  ...
  - **配对实战（round 1，判据）**：2400 场同牌山配对（两臂各 2 席、`seed 777001`、`--sample 64`、
    **54 分 02 秒 ≈ 0.74 场/秒**）→ **`hybrid@1` − 纯网 = +1.95 顺位点**
    ...
    `eval.py` 自己给的尺子是"检出 Δ=2 约需 2709 场"，所以现在正好卡在边缘：要钉成结论还得再跑 ~2700 场。
  ...
  - **臂 B（`hybrid@1` vs `teacher`，1200 场 / 1019.7 s = 1.18 场/秒）**：
    ...
    **强度效应未确立**（要钉 +2 需再 ~2700 场）。去向：混合留作未来在线 RL 的**保底**
...
- **P3 离线 RL（IQL → AWR）**（2026-09）：细节都写在 `python/mahjong_ml/{rewards,offline_rl,awr}.py`
  ...
  - **判据② 的实测（同牌山配对）**：**AWR − BC（`bc-003`）= +0.92 顺位点**
    ...
    **AWR − `teacher` = −1.16**（CI [−4.11, +1.86]、p=0.977、胜 601/负 599，1200 场）。
    → **未确立**：点估计为正，但 CI 宽 ±2.1（检出该量级需 ~2700 场），对 teacher 完全打平。
  ...
  - **判据② 两轮的实战（同牌山配对）**：`awr-001`（λ=0 critic）− BC = **+0.92**（p=0.391）、
    − teacher = −1.16（p=0.977）；`awr-002`（λ=1 critic）− BC = **+1.27**（**p=0.094**，胜 1241/负 1158）、
    − teacher = −1.66（p=0.885，胜 597/负 603）。→ **换排序更好的 critic 让效应变大但都没到显著**
    （要钉住 +1.3 需 ~2700 场/臂）。把之前几轮的尺子放一起：`BC − teacher ≈ −1.6`、
    `hybrid@1 − BC ≈ +1.95`、`AWR − BC ≈ +1.3` —— **全在 ±2 顺位点内、CI 全跨 0**。
