...
- **判据数字**（`AGENTS.md` §6.7 第 4 条"删完编译 + 跑自检"的实测）：清理后 **L1 `1268/0`、L2 `801/0`**
...
- **AUDIT 里登记的未闭合项**：`docs/AUDIT.md` §1.7.2「本轮仍未闭合并已登记为已知限制」
  （如 **S-58 的时序**：代码已按文档改对，但**缺自动化断言**）。动那块之前先读那条。
- **`docs/AUDIT.md` §1.8 的测试覆盖盲区**（`S-68`…`S-72`）已大部分闭合，剩下的写在同处。
- **shell 脚本（`server/pack/*.sh`）无法在本机执行**（Windows 上没有可用的 bash）：
  改动它们时只能做**静态审查 + 解析逻辑的独立验证**（例如拿真实 API 响应跑一遍
  `update.sh` 里的 `sed`/`grep` 链），并在真机（Ubuntu）上复验。**这条是环境限制，不是代码问题。**
