...
- **判据侧（两种口径，结论不同，都要看）**：
  ...
  - **同牌山 2+2 配对**（每代 800 场，逐场按牌山配对 + 自助法 CI）：g01 **+1.22** [−2.32,+4.69] ·
    g02 **−0.60** [−4.35,+3.16] · g03 **−1.37** [−4.95,+2.30] · g04 **−3.57** [−7.12,+0.00] · g05 **+1.10** [−2.41,+4.67]
    ⇒ **五个 CI 全部跨 0 = 证不出差别**（按 `eval.py` 的功效口径，800 场只够分辨 Δ≈2；点估计的抖动 ±2 顺位点）。
- **结论（如实说）**：v3 这一轮 5 代把模型做到了**teacher 附近**（阶梯口径上 g02/g03/g05 的小幅领先可分辨），
  但**同牌山配对口径下还没有可分辨的优势** —— 与 v2 谱系 `ppo2-g05`（+4.90 [+1.38,+8.46]，同样是 800 场）
  不是同一个量级。⚠ 两条谱系的**数据与配方都不同**（v2 从第 1 代就有 critic + 5 代 PPO；
  v3 是 BC 基模起步、第 2 代才补上 critic），所以不能直接比"谁的 g05 强"，只能说：
  **v3 要追上 v2 的强度，还得继续跑几代 / 把配对样本量加到几千场再判。**
