...
- **P4 世代循环与联赛**（2026-09）：`online.py`（采集→`--features`→紧凑集→PPO→导出）+ `league.py`
  ...
  - **配额又改过两次**（2026-09，都是用户指定）：`compact` 50 → **100 GB**；随后
    **2026-09-26 指定「S 盘上限 200 GB」** ⇒ 新增 **`TOTAL_QUOTA_GB = 200`**（数据根总量，只在 `raw`/`compact`
    里淘汰 —— 这两类"每代一份、可复算"），并把分项抬到**五项之和恰好 200**：
    `raw 30 → 80 GB`（≈44 代）、`compact 100 → 116 GB`（≈77 代）、`ckpt 2 / league 1 / logs 1` 不变。
    ⚠ 判据写成两条（`python/selfcheck.py`）：**总量 = 200**、且 **`sum(QUOTA_GB.values()) == TOTAL_QUOTA_GB`**
    —— 分项加起来够不到总量时，"总配额"就是一句空话。`report()` 现在也打
    `total=99.62GB/200GB`（当时实测）。⚠ 200 GB 是**策略上限**：S 盘物理 231 GB，真正的兜底仍是
    `MIN_FREE_GB = 10`（配额放大 ≠ 可以无限堆）。
...
- **训练端 C++ 引擎**（2026-09 起，`trainer/`）：同一份 JFR 剖面显示 **99% 的样本落在
  ...
  **发布版服务端一行不改**。路线是"先把向听/进张这条路径做快"（换算法为主、换语言为辅），
  ...
  - 两个"契约细节"值得记牢：① `Wall.doraIndicators()/uraIndicators()` 返回的是 **kind** 而且
    ...
    **3 轮 × 4 家 × 一次抓 4 张（共 12）** → 每人补 1 张（13）→ 庄家第 14 张 → `finishDealing`，
    手牌按 `Round.compareTile`（先 kind、再"赤五在前"、最后比 id）排序。
    ⚠ **2026-09 修正：②原来写成"13 巡 × 4 家 × 1 张"，是错的** —— 探针自己重写了配牌顺序、
    C++ 照着探针写，于是**两边一起错还互相印证了 512 组**（假阳性）。判据：**探针只准调 Java
    自己的入口**（`new Round(...)` + `debugSetup()` 再读 `hand[]`），不许在探针里重实现规则 ——
    否则"对拍"就退化成"和我的重实现比"。同一条教训在振听那一轮也适用（那边一开始也是想自己抄一遍）。
  ...
  - ⚠ 构建脚本的增量指纹**必须含头文件**：只盯 `.cpp` 时改 `wall.hpp` 不会重编（踩过一次，
    表现为"改了代码但对拍结果没变"）。
   - **M1（向听/进张/听牌形）已完成**（`counts.hpp` / `shanten.hpp/.cpp` / `handeval.hpp/.cpp`
     + `tools/RuleProbe.java` + `tools/trainer-rule-parity.mjs`）：**100 万手向听 + 21.7 万手
     派生评估（523,413 行打牌后评估）逐字段一致**；纯计算口径下向听 **31.5×**、进张 **19×**、
     听牌形 **44×**、`afterDiscard` **29.7×**（表格与命令见 `docs/TRAINER-CPP.md` §6.1）。
   - **M1 的性能关键不是"查表"而是"位并行合并"**：第一版按"每组一个 (面子,搭子,雀头) 可达集、
     四组两两三重循环卷积"写，单次合并 2500 次迭代 → 只比 Java 快 **3.9×**（`of` 随机手）。
     改成"可达集压成 64 位掩码（位号 `s*10 + q`）+ `out |= (b & rectDom[s][q]) << (s*10+q)`"
     之后，一次合并只剩 ~25 次移位 —— 向听路径到 **31.5×**。⚠ 位号**乘 10 不能乘 5**：
     只有 `q_a + q_b ≤ 4 < 10` 才保证位移不进位到 `s` 位。
   - **对拍发现的 Java 侧隐患**：`Agari.addWinVariants` 在 `melds == null` 时
     `for (Meld m : melds)` 直接 NPE（而同方法开头的 `meldCount = melds == null ? 0 : melds.size()`
     说明它本该容忍 null）。**线上打不到**（`ObsFeatures` 与 `Bot` 传的都是列表），所以按
     "服务端一行不改"的口径**不动它**，但探针必须传**空列表**而不是 `null`。
   - **`waitShapes` 的"不是听牌张"必须编码成 `-1`**（Java 未听牌时是 `null`，哈希按全 `-1` 走）：
     C++ 第一版只在听牌时 `fill(-1)`，未听牌时是 `std::array{}`（全 0）→ **8 个标量字段全一样、
     只有数组哈希对不上**（138/200 行红）。修法：`evalOf` 一进来就 `fill(-1)`，
     让"34 维听牌形"在任何时候都是一个意思。
   ...
   - **M2 中段（种子链 + 精算/连庄判据）已完成**（`seed.hpp` / `roundscoring.hpp/.cpp` +
     `tools/SettleProbe.java` + `tools/trainer-settle-parity.mjs`）：**21.36 万行逐位一致**，
     文档例子逐字复现（M.League 53600/28600/20000/−2200 → +73.6/+8.6/−20/−62.2；《雀魂》同样点数
     → +43.6/+8.6/−10/−42.2）。表格与四条实现要点见 `docs/TRAINER-CPP.md` §6.3。
   - **为什么这一层值得单独对拍**：`rank_points` 是**奖励函数的直接输入**（`rewards.py` 直接读、
     从不自己重算 uma），而它由一串"看起来都很简单"的判据堆出来（连庄/本场/和了止/延长战/同点拆分/
     终局余棒）。这些判据在 Java 侧原来散在**五个地方各写一行**（AUDIT S-46/S-54/S-55/S-64），
     每处写错都只表现为"分数算错但不报错"。
   - **三条容易"顺手写错"的地方**：① `mixSeed`（先加黄金比）、`SelfPlay.seedFor`（**不加**黄金比）、
     `nextRoundSeed` 的确定性岔路是**三个不同的函数**，长得像但不能统一；② 所有移位必须在
     **无符号域**做（Java `>>>` 是逻辑右移且 long 溢出有定义，C++ 有符号溢出是 UB）；
     ③ `splitRemainder` 必须**向下取整**（`Math.floorDiv`）—— 末位顺位点是负的，截断除法会让尾数分不完。
   ...
   - **M2 下半之二（鸣牌仲裁判据）已完成**（`roundclaims.hpp` + 五种语料行接进 `trainer settle` /
     `SettleProbe.java`）：等级/压过/荣和收齐/收工/回包认领 **12,567 行一致**（§6.5）。
   - **这一层最容易写错的是"提前收工改变赢家"**（AGENTS §2.3-10）：① 等级尺子
     「荣和 3 > 杠 2 > 碰 1 > 吃 0 > pass −1」必须与真实仲裁循环同一把；② **同级看座次距离**
     （离打牌者近的赢）；③ **荣和要收齐**（多荣和）—— 但"谁都不能荣和"时这条**不成立**，
     否则会立刻收工、把碰/吃的机会整片丢掉；④ `askedBestRank` **缺项要保守继续等**。
     ⚠ 自检写这条时踩了一次：要走到 `canBeat` 那一步，`ronCapable` 里必须有**未答、又不在 asked 里**
     的座位 —— 否则要么前面 `allRonAnswered` 直接收工、要么循环里因为"可能荣和"继续等。
   ...
   - 这一块是**观测特征（M3）的底座**：`Observation.visible`、进张枚数、危险度必须**同源**，否则
    ...
    - **数百场规模通过，并修掉校验器一条过严规则**：`300 场 × 2 小局 × first × seed 20260101` →
      ...
      **DATASET PASS**（300 场 / 600 小局 / 52,856 决策）。过程中报红的那一处（`g63` 第 2 小局只有
      2 次决策）经查是**校验器过严**：它要求"每小局 ≥ 4 次决策"，而一小局完全可以在 1~2 次内结束
      （庄家九种九牌中途流局 / 第一张舍牌即被荣和）—— 而且**同一份文件 Java 自己产出的也 FAIL**
      （两份逐字节相同，177,071 B）。规则已改成"**有小局就必须有决策**"，并留负向对照（删掉某小局
      全部决策 → 仍报红 exit 1）。
    - **性能口径修正（重要）**：先前"单核 21.2× / 8 workers 4.7×"是**短局（2 小局）**测的，含 JVM 启动开销、
      会放大倍数。换**真实工作量**（100 场**完整半庄** × `first` × 24 workers）：Java **10,558** 决策/秒
    ...
    - **案例：「自家回合 `legal` 为空」是两侧同源的退化（训练端 takeover 期的真 bug）** ——
      ...
      频率（**实测**）：`first` **3/500 场 ≈ 1/167**、`random` **0/500** —— 它**跟策略走**（`first` 会贪心吃成
      食替锁死的形），而且与 `--workers` 无关（8w 与 1w 的兜底计数完全相同 ⇒ 只由种子决定）。
      首案：`seed 20260101` → 第 66 场（第 300 次决策）、`seed 31337` → 第 119 场；
      ...
      **判据教训**：约 1/167 频率的退化**测不到**在"100 场"这个数字上 —— 完整半庄验收**至少 200 场**
      （已跑到 **500 场 × 2 策略**：与 Java 各 500/500、与串行各 500/500、失败 0）；
      而且"对拍 C++ 自己的串行/并行"永远发现不了它（`--workers` 确实没改产出，问题在**引擎**），
      **必须同时跑 Java 侧参考**。
    - **案例：把网络前向搬进训练端（M3 后半，2026-09）** —— 目标：P5 对抗训练的每一席都是 `net:<ckpt>`，
      没有它 C++ 生产者只能跑基线臂。要点：
      ① **真正缺的不是前向，而是"观测 → 网络输入"的拼装**：`Features.state(607)` / `candidate(96)`
      原先只有 Java/Python 有（C++ 的 sidecar 是 int 段），而它依赖的 `perDecision(68)` / `perCandidate(8)`
      早已在 C++ 且逐字节验过（sidecar 200/200）—— 所以这一轮是"在已验的地基上叠一层 + 约 60 行前向"。
      ② **必须 `-ffp-contract=off`**（已加进 `build.ps1`）：FMA 收缩只改最后一位舍入，而 1e-4 的逐元素容差
      **挡不住** argmax 翻转；判据是"同种子 → 逐字节轨迹"。实测：golden 小网（hidden 32/head 16）
      Java↔C++ **maxΔ = 0（逐位相同）**；真实权重（`ckpt/ppo2-g04/net.bin`，266,753 参数）
      **36,181 条决策 maxΔ = 1e-6、`n` 与 argmax 全部相同**。
      ③ **端到端才是判据**：`net:` 贪心 **100 场完整半庄逐字节 100/100**（同 24 核 Java 85.6 s vs C++
      **4.2 s = 20.1×**）；`net:@0#1.0` 温度采样（世代采集的形状）**20 场逐字节 20/20** —— 即 `exp` /
      `nextDouble` / CDF 累积那条路也与 Java 一致；`--sample 7` 组合同样 PASS。
      ④ **两个坑**：`%.9g` 的**文本**必然不同（Java 保留尾随零 `-554.013000`、C 去掉 `-554.013`），
      所以闸门按**数值 + 容差**比、文本差异只当 info；**Windows 命令行上限 32,767 字符** ——
      整个 `bc-001`（800 个路径）一次传给探针**根本起不来**（Node 报 `status=null`，看着像被信号杀），
      闸门已改为按长度自动分批（`NET_PARITY_MAX_ARG_CHARS` 可强制小批测试）。
      ⑤ **仍然缺 `teacher`**：联赛设计里"老师常驻一席"，所以含 teacher 席的世代仍只能在 Java 生产者上跑
    ...
    - **案例：把 `teacher`（1,443 行五层取舍）搬进训练端（M3 收尾，2026-09）** —— 它是"对抗训练上 C++"的
      ...
      **danger 重构没动特征**（sidecar 30/30）。
      ...
      ⑤ **一代的成本（2000 场/代，C++ 生产者）**：采集 **40.3 s** + 派生特征 9.9 s + 紧凑集 **452.6 s**
      + PPO 4 epochs 125.7 s + 导出 3.1 s ≈ **10.5 分钟/代**；Java 同代 ≈22–24 分钟（只慢在采集那 727–833 s）
      ⇒ **瓶颈已从采集转到紧凑集（占 72%）**。判据侧（`online ladder`：每代 1200 场 Elo + 800 场 2+2 配对）
      5 代共 10,000 场、采集只用 **145 s**。采集吞吐：4 策略混桌 24 workers **78–83 场/秒**、
      纯 `teacher×4` ≈135 场/秒、P5 世代形状（16 workers）**49.6 场/秒** —— 相对 Java 的 1.42 场/秒是
      **35–95×**（`docs/TRAINING.md` §1 与 §4 P5、`docs/TRAINER-CPP.md` §6.18）。
...
**`online.py` 的两个续跑陷阱（2026-09-26 实测踩到，都已修）**

- **`--generations` 是「跑到第几代（含）」而不是「再跑几代」**：实现是
  `for g in range(start_gen, generations + 1)`。所以"只跑第 2 代"要写
  `--start-gen 2 --generations 2`；写成 `--generations 1` 那是**空循环** ——
  它照样打印"1 代跑完；台账：…"，但**什么都没跑、台账没变**（我是靠"新 ckpt 目录不存在"才发现的）。
  help 文案已改成显式警告 + 例子。
- **`--init-value`（IQL critic 冷启动）原来写死 `if g == 1`**：`--start-gen 2` 续跑时 g 从 2 起
  ⇒ 冷启动**被静默忽略**（跑完价值头还是上一代的）。现在条件是 `g == start`
  （"这一跑的第一代"），续跑同样生效 —— 语义没变（一代之后 value 都靠 `--init` 继承）。

**紧凑集构建的并行化（2026-09-26，用户点名"CPU 还有空余"带出来的）**

- **背景**：一代 2000 场的预算里采集只要 **40 s**，而 `dataset build` 是**单核 Python ≈500 s**
  （占整代 ≈11 min 里的 8 分钟）—— `--workers` 从前只作用于采集与派生特征，**这一步才是 P5 的瓶颈**。
- **做法**：按**文件分组**切成 ≤workers 块，各块写 memmap 的**不同行切片**；行号只由
  "文件顺序 + 文件内行序"决定 ⇒ 产物**与并行度无关**（不是"大概一致"）。
- **两个环境坑**（都会静默变慢或直接炸，值得记）：
  ① 本机沙箱**禁命名管道** ⇒ `multiprocessing.Pool` 在构造时就 `PermissionError: [WinError 5]`
  （`_winapi.CreateFile`），改用**无管道子进程 + req/res 文件 + 退出码判据**；
  ② 沙箱**拒绝对 `mkdtemp` 新建目录的写入**（`mkdir` 建的目录正常）⇒ scratch 用 `mkdir` 固定名。
- **加速与判据**：800 场 / 279,518 条决策 —— 串行 **88.2 s** vs 8 workers **15.0 s**（**5.9×**）；
  两个来源（`bc-001`+`control-r1`，434,791 条）串行约 185 s vs 12 workers **25.2 s**（**7.3×**）。
  **逐字节相同**：同语料 23 个文件 **0 差异**、`--max-decisions` 截断路径 **0 差异**，
  并且**逐字节复现了训练实际用过的那份** `compact/bc-v3-001`。闸门钉在 `python/selfcheck.py`（231 项）。
- ⚠ 这条只改**调度**：`meta.json` 里的 `split_seed`/`train_files`/条数一字不变，已训过的数据集不受影响。

**特征 v3 谱系第一轮：g01–g05（2026-09-26）**

- **起步**（v3 没有历史权重可续，必须重建基模）：`compact/bc-v3-001`（`bc-001`+`control-r1`，434,791 训练条）
  → BC 60 epoch / **365.9 s**，val top-1 **0.876**（类型 0.988）；对照 v2 时代同量级 `bc-005`（441,981 条）**0.8607**
  —— ⚠ 不是受控 A/B（数据来源与切分不同），只当"v3 至少不差"的迹象。
- **IQL critic**：`iql-v3-001`，20 epoch / **621.1 s**（`--rank-weight 1 --select-metric val_ep_pearson`）：
  val MAE **6.0905**（恒 0 基线 6.2214 / 恒均值 6.2225 ⇒ **都赢**）、逐点 ρ +0.613、**小局级 ρ +0.677**、
  Spearman +0.327；只看有收支的小局 ρ **+0.708**、MAE 7.5312（恒 0 8.5399）。
- **每代的成本与价值头（判据在价值诊断那一列，不在策略损失）**：

  | 代 | 采集 | 紧凑集 | PPO 4ep | 价值头 val MAE（常数基线） | 逐点 ρ / episode ρ |
  | --- | --- | --- | --- | --- | --- |
  | g01 | 43.9 s | 479.2 s | 124.1 s | 7.5772（6.22） | +0.409 / +0.421 |
  | g02（**critic 冷启动**） | 47.1 s | 516.7 s | 74.9 s | **5.2243**（6.21） | +0.732 / +0.759 |
  | g03 | 42.2 s | 514.3 s | 78.7 s | 5.1619（6.34） | +0.744 / +0.763 |
  | g04 | 38.4 s | 459.6 s | 71.4 s | **4.9116**（6.11） | +0.759 / +0.773 |
  | g05 | 51.0 s | **45.0 s**（并行） | 68.8 s | 5.3894（6.22） | +0.697 / +0.736 |

  ⇒ **价值头从"打不过常数基线"变成"显著赢"**，就是那一步 IQL critic 冷启动（g01 的价值头是冷的）。
  并行化后**整代 ≈3.6 min**（原 ≈11 min）。
- **判据侧（两种口径，结论不同，都要看）**：
  - **Elo 阶梯**（6000 场 / 8 策略，Plackett-Luce）：`g05 +1.018`（θ−teacher **+0.264**，半宽 0.144 ⇒ **不含 0**）
    > `g02 +0.963 (+0.209)` > `g03 +0.917 (+0.163)`（也都 > 半宽）> `g04 +0.876 (+0.121)` >
    `g01 +0.847 (+0.093)` > `teacher +0.754`（anchor）> `first −2.675` ≈ `random −2.700`。
  - **同牌山 2+2 配对**（每代 **两种座位配置各 800 场**，再合成 n=1600；逐场按牌山配对 + 自助法 CI）。
    ⚠ **为什么必须两组**：轮转是 `src = (seat + game) % 4`，它**保持策略表的块结构** ——
    `A,A,B,B` 只采样"两席相邻"（{0,1}/{0,3}/{1,2}/{2,3} 各 1/4 场），
    `A,B,A,B` 只采样"两席对家"（{0,2}/{1,3} 各半）。单跑一组等于把另一种座位关系**留成未采样偏差**
    （2026-09-26 用户指出"2+2 的坐位顺序"后补上，实现是 `online._arrangement_specs`，
    座位覆盖实测：相邻 run = 四个相邻对各 50/200 场、对家 run = {0,2}×100 + {1,3}×100）。

    | 代 | 相邻座位 Δ [CI] | 对家座位 Δ [CI] | **合计 n=1600 Δ [CI]** |
    | --- | --- | --- | --- |
    | g01 | +1.22 [−2.32,+4.69] | −0.75 [−4.28,+2.74] | +0.24 [−2.21,+2.74] |
    | g02 | −0.60 [−4.35,+3.16] | +0.17 [−3.42,+3.76] | −0.21 [−2.84,+2.37] |
    | g03 | −1.37 [−4.95,+2.30] | +0.38 [−3.21,+3.94] | −0.49 [−3.06,+2.07] |
    | g04 | −3.57 [−7.12,+0.00] | −0.59 [−4.12,+2.99] | −2.08 [−4.56,+0.39] |
    | g05 | +1.10 [−2.41,+4.67] | **+4.23 [+0.67,+7.82]**（p=0.044） | **+2.66 [+0.14,+5.18]**（p=0.068） |

    ⇒ ① **只有 g05 的合计 CI 不含 0**（它同时也是 Elo 阶梯第一）—— "v3 第 5 代强于 teacher"现在有了一条
    **边缘**证据（p=0.068；⚠ bootstrap CI 与配对 t 的 p 值在这里不一致，别当成"钉住了"）。
    ② **座位配置确实会改变结论**：g05 相邻 +1.10（不显著）vs 对家 **+4.23**（p=0.044）；
    g01 正好相反（+1.22 → −0.75）。两种配置之差本身在这个样本量下也**不显著**（SE≈2.6），
    但"只跑相邻"会把结论写成"五代全跨 0"。③ 合计 CI 半宽 ≈±2.5 vs 单配置 ≈±3.5，这就是两配置一起判的收益。
- **结论（如实说）**：v3 这一轮 5 代把模型做到了**teacher 附近偏上**——Elo 阶梯上 g02/g03/g05 的小幅领先
  可分辨，同牌山 2+2（两种座位配置合计）**只有 g05 边缘通过**（+2.66 [+0.14,+5.18]）；其余四代合计仍跨 0。
  与 v2 谱系 `ppo2-g05`（+4.90 [+1.38,+8.46]，同样是 800 场**单配置**）不能直接比量级 ——
  ⚠ 两条谱系的**数据与配方都不同**（v2 第 1 代就有 critic + 5 代 PPO；v3 是 BC 基模起步、第 2 代才补 critic），
  而且 v2 那次只跑了相邻配置。真要比，得用**同一套双配置协议 + 几千场**重测两条谱系。
- ⚠ 别拿阶梯自对弈汇总表里的"网络 vs teacher"当结论（那张表是 1 网 + teacher + first + random 的混桌单席，
  两个弱对手会把网络的顺位点抬起来）：**判据只有配对/阶梯的 CI 那一列**。

**特征 v3：模型到底"看不看得见自己"（2026-09-26，用户提问引出的一次完整审计）**

- **起因**：用户问"现在的模型能感知到自己所处的位置、点数、其他人的点数吗"。结论是**位置部分能、自己点数不能、
  其他人的点数能但没有"我"这个标签** —— 这不是训练不足，是**输入里缺锚**。
- **结构性证据**（读 Java 权威 + Python 镜像 + 三个探针）：
  ① 607 维里与位置/点数有关的只有 `round/5`（场风·局·本场·**庄家偏移**·供託）与 `scores/4`，共 9 维；
  ② `seat` **本身不进向量**，全仓只有两处用到它：`(dealer − seat) mod 4`（Features.java:144）与
  鸣牌来源 `(from − seat) mod 4` 的 one-hot（:168）；没有任何"我这一格"的标记；
  ③ 探针：构造两个**各自自洽**的局面（我是 0 号 vs 1 号），607 维输入**逐位相同（maxΔ = 0）**，
  而"我的点数"是 31000 vs 21000；真实轨迹行上把 `seat` +1、庄家/来源同步 +1，输入同样**逐位相同**；
  ④ 反证：只把 `scores` 数组环移一位 → 向量变化 ⇒ 四个点数**确实在输入里**，缺的只是标签。
- **实测"网络自己能不能推出来"**（真实语料 `raw/ppo2-g05/g468.jsonl`，1545 条决策；用网络拿得到的三条线索：
  账目恒等式「手牌数 = 13 + 摸牌数 − 自家舍牌数 − 自家副露出张」+ `menzen` ↔ 四家副露 + `self_riichi` ↔ `riichi[]`）：
  **唯一锁定且正确只有 15.5%**，候选 2 家 20.2% / 3 家 29.4% / **4 家全自洽（完全不可分）30.7%**，
  开局阶段（总舍牌 ≤ 4）**90.7% 完全不可分**；真值命中 94.3%（账目模型 ±1 容差，含 1.5% 无解）。
  另：把 `scores` 环移一位后，训练好的 `ppo2-g05` 在同样 1545 条里 **334 条（21.6%）argmax 改变**
  ⇒ 网络确实在"读"这四个数（按"点数 ↔ 某家牌河/立直"的对齐），只是读不出哪个是自己的。
- **判据**：*做不到按点数/顺位的押し引き*（"我末位必须打点"、"我领先可以弃和"）是**结构性**的；
  而 teacher（`Bot` §6.6 第五层）恰恰用点数/顺位做取舍 ⇒ 这部分老师行为学生**只能学成平均**。
- **v3 落地（`Features.VERSION 2→3`）**：
  ① **四家块旋转到自己为下标 0**（`meld_kind/meld_tiles/river/riichi/ippatsu/scores` 的第 j 格 = 座位 `(seat+j)%4`）
  ⇒ 一次性消掉"哪一格是我"（自己的牌河/副露/立直也变得可辨识）；
  ② 新增 **`points/5`**：自己点数、与三家均值差、**顺位**、与上一名/下一名的分差；
  ③ 派生段 68 → **71**：`[shanten_now, value_han, value_points]`，其中打点粗估就是 **teacher 押し引き的那把尺子**
  （`Bot.estimatedHan` 提到了 `rules/HandEval`，两边共用一份，不再各写一份）；
  ④ obs 新增 `kuitan`（规则取舍里唯一被特征用到的位；打点粗估的断幺项要它）⇒ `Observation.VERSION 1→2`。
- **一个被实测否决的设计**：本来还想在状态段放"当前进张种数/枚数"。它要跑一次完整 `HandEval.of`
  （34 次 `Shanten.min`），实测 **≈0.94 ms/决策**（状态段 26 µs → 1 ms，40 倍）；而"打完这张之后"的进张
  本来就在**逐候选段**里，决策时比的是候选之间的大小 ⇒ **信息并不缺**，所以砍掉。
  ⚠ 教训：往状态段加任何派生量之前，先按 `docs/TRAINING.md` §3.3 那张表量一遍单线程开销。
- **sidecar 格式变更**：逐决策段 **uint8 → int16**（`TraceFeatures` 的 A 段）。原来 68 维都是 0..100 装得下，
  v3 带了点数（最大 32000）⇒ uint8 会**截成 mod 256**；症状是 golden 对拍 `maxΔ = 0.0545`（= 2000→208 的差）。
  Python 侧 `load_sidecar` 同步改成 `<i2`（`off_b = 20 + ndec*pdec*2`）。
- **兼容性代价（必须知道）**：`net.bin` 头部记着 state/cand 维度 ⇒ **老的 v2 权重（607 维）在新服务端上会被
  `NeuralPolicy` 明确拒绝**（"权重维度 (607,96) != 本服务端特征维度 (615,96) —— 重新导出权重（特征版本 3）"，构造期报错、不静默）；
  同理 `compact/*` 按 607 维落盘 ⇒ 旧紧凑集作废，P5 世代要重算重训。已发布的 `ppo2-g05` 包属于 v2 代，
  要用 v3 就得重训 + 重导出 + 重发包（见 `docs/BOT-AI.md`）。
  ⚠ **旧 sidecar 也是硬拒绝**：`g*.feat.bin` 头部带着 `derived_version` 与段长，v1/68 维的旧文件在
  `dataset build` 时会直接报「派生特征版本 1 != Python 侧 2」/「派生块布局不符：perDec=68」——
  **重跑一次 `--features` 就整体重建**（本机实测 300 文件 / 16 万决策 ≈13 s，单 worker）。
- **审计里被判"不必补"的项（连同理由，免得下次再翻一遍）**：
  ① 自家**进张种数/枚数** —— 见上一条实测（≈0.94 ms/决策，且逐候选段已有"打完之后的进张"）；
  ② 自家**振听种类**（同巡/舍张/立直）与**自家一发**：`Bot` 源码里 `furiten`/`ippatsu` **零引用**
     （振听已有布尔位，防守口径走 `Danger`），补进去只是多两维它用不上、也推得出的位；
  ③ **立直宣言巡目**：`Danger.worst` 的签名里没有它（teacher 自己的危险度模型也不用）——
     补了会造出"特征比老师更聪明"的口径分裂，反而让标签对不上；
  ④ 自家**宝牌/赤宝数**：`hand_red` + `dora_indicators` 已在基础段，逐候选 `dora_count` 实测非零占比
     **40.8%**（最大 3）⇒ 信息不缺，不重复放；
  ⑤ **场风/自风**：`round/5` 里的 `(dealer − seat)` **就是自风**（庄家=东），场风是 `bakaze` —— 我一开始也想加，
     对着 `Bot.isYakuhai` 读了一遍才发现是重复的；
  ⑥ **完整规则集**：只补了特征真正用到的一位 `kuitan`；uma / 食替 / 流局口径仍不进 obs
     （与 `NOTES.md` §7「策略看不见"场外"规则设定」同一条已知限制）。
- **本轮的验收**：L1 `--selftest` **1370/1370**、`python selfcheck.py` **228/228**、
  golden 夹具按 v3 重导（state 615 / derived_version 2，Java↔Python 逐元素一致）；训练端 C++ 侧
  `trainer --selftest` PASS、`trainer-selfplay-parity` 逐字节（`pass` 1,119,860 B / `net:<v3>` 159,477 B）、
  `trainer-features-parity` 3 个 sidecar 逐字节（203,988 / 331,636 / 163,844 B）、
  `trainer-net-parity --golden` **maxΔ = 0**（6/6 argmax 同），C++ `--features` 吞吐只降 **≈2.5%**
  （84.1 µs/决策，噪声内）—— 全部数字与判据见 `docs/TRAINER-CPP.md` §6.19。
